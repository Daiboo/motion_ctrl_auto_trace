...
lpf_param accel_lpf_param,gyro_lpf_param;   // 加速度和陀螺仪低通滤波器参数
lpf_buf gyro_filter_buf[3],accel_filter_buf[3];




sensor smartcar_imu;   // 小车传感器参数
...
void trackless_ahrs_update(void)
{
	...
	if(smartcar_imu.rpy_deg[_YAW]<0.0f)   smartcar_imu.rpy_deg[_YAW]+=360.0f;
	if(smartcar_imu.rpy_deg[_YAW]>360.0f) smartcar_imu.rpy_deg[_YAW]-=360.0f;
...
void imu_temperature_ctrl(void)
{
	...
	temp_integral+=temp_ki*temp_error*0.1f;
...
void simulation_pwm_init(void)
{
	...
	HWREG(GPIO_PORTC_BASE + GPIO_O_LOCK) = GPIO_LOCK_KEY;//解锁PC2
	HWREG(GPIO_PORTC_BASE + GPIO_O_CR) |= GPIO_PIN_2;//确认
	HWREG(GPIO_PORTC_BASE + GPIO_O_LOCK) = 0;//重新锁定
  
	HWREG(GPIO_PORTC_BASE + GPIO_O_LOCK) = GPIO_LOCK_KEY;//解锁PC3
	HWREG(GPIO_PORTC_BASE + GPIO_O_CR) |= GPIO_PIN_3;//确认
	HWREG(GPIO_PORTC_BASE + GPIO_O_LOCK) = 0;//重新锁定

	SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOJ);
	GPIOPinTypeGPIOOutput(heater_port,heater_pin);
	GPIOPinWrite(heater_port,heater_pin,0);
}
...
void simulation_pwm_output(void)
{
	...
	uint16_t width=temp_output;
	static uint16_t cnt=0;
	cnt++;
	if(cnt>=Simulation_PWM_Period_MAX)  cnt=0;
	if(cnt<=width) GPIOPinWrite(heater_port,heater_pin,heater_pin);
	else GPIOPinWrite(heater_port,heater_pin,0);
}
...
void get_wheel_speed(void)
{
	...
	smartcar_imu.left_motor_speed_cmps =get_left_motor_speed();   	// 左轮速度
	smartcar_imu.right_motor_speed_cmps=get_right_motor_speed();	// 右轮速度
	//计算两轮平均速度
	smartcar_imu.state_estimation.speed=0.5f*(smartcar_imu.left_motor_speed_cmps+smartcar_imu.right_motor_speed_cmps);
	smartcar_imu.state_estimation.azimuth=smartcar_imu.rpy_deg[_YAW];  // 偏航角
